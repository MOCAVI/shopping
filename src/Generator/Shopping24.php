...
namespace ElasticExportShopping24\Generator;

use ElasticExport\Helper\ElasticExportCoreHelper;
use ElasticExport\Helper\ElasticExportPriceHelper;
use ElasticExport\Helper\ElasticExportStockHelper;
use ElasticExportShopping24\Helper\PriceHelper;
use ElasticExportShopping24\Helper\PropertyHelper;
use ElasticExportShopping24\Helper\StockHelper;
...
class IdealoDE extends CSVPluginGenerator
{
    ...
    protected function generatePluginContent($elasticSearch, array $formatSettings = [], array $filter = [])
    {
        ...
        if($elasticSearch instanceof VariationElasticSearchScrollRepositoryContract)
        {
            ...
            do
            {
                ...
                $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.writtenLines', [
                    'Lines written' => $limit,
                ]);
                ...
                $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.esDuration', [
                    'Elastic Search duration' => microtime(true) - $esStartTime,
                ]);

                if(count($resultList['error']) > 0)
                {
                    $this->getLogger(__METHOD__)->error('ElasticExportShopping24::item.occurredElasticSearchErrors', [
                ...
                if(is_array($resultList['documents']) && count($resultList['documents']) > 0)
                {
                    ...
                    foreach($resultList['documents'] as $variation)
                    {
                        ...
                        {
                            $this->getLogger(__METHOD__)->info('ElasticExportShopping24::item.itemMainVariationAttributeNameError', [
                        ...
                        {
                            $this->getLogger(__METHOD__)->info('ElasticExportShopping24::item.itemExportNotPartOfExportStock', [
                                'VariationId' => (string)$variation['id']
                            ]);
                    ...
                    $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.buildRowDuration', [
        ...
        $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.fileGenerationDuration', [
    ...
    {
        $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.itemExportConstructGroup', [
            'VariationGroup' => count($variationGroup) . ' variations to be printed in CSV'
        ]);

        foreach($variationGroup as $variation)
        {
            $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.itemExportConstructItem', [
                'VariationId' => (string)$variation['id']
            ]);
        ...
        $this->getLogger(__METHOD__)->debug('ElasticExportShopping24::item.itemExportConstructGroupFinished', [
    ...
    private function buildRow(KeyValue $settings, $variation)
	{
        ...
        try
        {
               ...
               if($checkoutApproved == 'true')
                {
                    ...
                    {
											//SKU soll nicht geschrieben werden
                      //ADK  $sku = $this->elasticExportCoreHelper->generateSku($variation['id'], self::IDEALO_CHECKOUT, 0, $variation['id']);
                    }
            ...
            /*ADK        if($this->propertyHelper->getProperty($variation, self::PROPERTY_IDEALO_SPEDITION) === true)
                    {
                ...
                else
                {
                    ...
                    {
                    //ADK    $sku = $this->elasticExportCoreHelper->generateSku($variation['id'], self::IDEALO_DE, 0, $variation['id']);
                    }
                ...
                if(count($this->usedPaymentMethods) == 1)
                {
                    foreach($this->usedPaymentMethods as $paymentMethod)
                    {
                        foreach($paymentMethod as $method)
                        {
                            ...
                            {
                                $this->getLogger(__METHOD__)->error('ElasticExportShopping24::item.loadInstanceError', 'PaymentMethod');
                            }
                        }
                    }
                }
            ...
            {
                $this->getLogger(__METHOD__)->info('ElasticExportShopping24::item.itemExportNotPartOfExportPrice', [
                    'VariationId' => (string)$variation['id']
                ]);
            }
        }
        ...
        {
            $this->getLogger(__METHOD__)->error('ElasticExportShopping24::item.fillRowError', [
